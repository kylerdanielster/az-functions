...
/// Activities read payment data from the BatchPayments table via <see cref="IBatchPaymentStore"/>.
/// </summary>
public class BatchOrchestration(IHttpClientFactory httpClientFactory, ISftpClientFactory sftpClientFactory,
    IGLErrorQueue glErrorQueue, IBatchPaymentStore batchPaymentStore)
{
        ...
        [OrchestrationTrigger] TaskOrchestrationContext context)
    {
        ...
        var input = context.GetInput<BatchOrchestrationInput>()
            ?? throw new InvalidOperationException("Orchestration input is missing.");

        logger.LogInformation("[Batch] Orchestration {id} started — batch {batchId}.",
            id, input.BatchId);

        // Step 1: Create and upload payment file
        logger.LogInformation("[Batch] Orchestration {id} — creating payment CSV...", id);
        string paymentContent = await context.CallActivityAsync<string>(nameof(CreatePaymentFile), input.BatchId);

        string paymentFileName = $"payment_{input.BatchId}.csv";
        ...
        catch (TaskFailedException ex)
        {
            ...
            try
            {
                await context.CallActivityAsync(nameof(SendCallback),
                    new SendCallbackInput(input.CallbackUrl, new BatchCallback(input.BatchId, BatchStatus.Error)),
        ...
        string glContent = await context.CallActivityAsync<string>(nameof(CreateGLFile), input.BatchId);

        string glFileName = $"gl_{input.BatchId}.csv";
        ...
        catch (TaskFailedException ex)
        {
            ...
            try
            {
                await context.CallActivityAsync(nameof(SendToGLErrorQueue),
                    new GLErrorMessage(input.BatchId, input.CallbackUrl, ex.Message));
            }
        ...
        {
            await context.CallActivityAsync(nameof(SendCallback),
                new SendCallbackInput(input.CallbackUrl, new BatchCallback(input.BatchId, BatchStatus.Processed)),
                CallbackRetryOptions);
        }
        catch (TaskFailedException ex)
        {
    ...
    public record GLErrorMessage(string BatchId, string CallbackUrl, string ErrorMessage);

    // --- Activities ---

    /// <summary>Activity that reads payments from Table Storage and builds a payment CSV.</summary>
    [Function(nameof(CreatePaymentFile))]
    public async Task<string> CreatePaymentFile([ActivityTrigger] string batchId, FunctionContext executionContext)
    {
        ILogger logger = executionContext.GetLogger(nameof(CreatePaymentFile));

        var payments = await batchPaymentStore.GetPaymentsAsync(batchId);
        ...
        logger.LogInformation("[Batch] Created payment CSV for batch {batchId} ({count} payments).", batchId, payments.Count);
        return sb.ToString();
    }

    /// <summary>Activity that reads payments from Table Storage and builds a GL CSV (omits sensitive banking fields).</summary>
    [Function(nameof(CreateGLFile))]
    public async Task<string> CreateGLFile([ActivityTrigger] string batchId, FunctionContext executionContext)
    {
        ...
        logger.LogInformation("[Batch] Created GL CSV for batch {batchId} ({count} payments).", batchId, payments.Count);
        return sb.ToString();
    }
